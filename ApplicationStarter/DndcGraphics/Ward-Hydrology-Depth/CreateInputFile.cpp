...
void CreateDndcInputFiles(char *InputFileName)
{
	...
	float m_LagoonCapacity, m_LagoonSurfaceArea, LagoonToField[10], LagoonToMarket[10], LagoonRemain[10], LagoonManureAplDepth[10];
	int m_LagoonCover, m_LagoonRain, m_LagoonDrainTimes, LagoonDrainMonth[10], LagoonDrainDay[10], LagoonManureAplMethod[10];
...
*/
	...
	else
	{
		...
		if( Sks<0.015)  Sks = 0.015;//sks default 0.015
		MissFlag=0;
	}
	...
	for(int ix8=1; ix8<=n; ix8++)
		{
			...
			fscanf(db, "%f %f %f %d %f %f", &ManuN[ix8], ManureNH4[ix8], ManureNO3[ix8], ManureAppHour[ix8],ManureDM[ix8],ManureAppHour[ix8]);
		}
	fclose(db);
