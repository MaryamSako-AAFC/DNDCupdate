...
int ReadSoilFeature(CString mode)
{
    ...
    if(DSks<0.008) DSks=0.000015;//default sks 0.015 was 0.008
...
//	IData.H = 0.02;//test a fixed depth
    if (IData.H <= 0) {
...
int SaveInterFarmParas()
{
	...
	int ManuMonth[300], ManuDay[300], ManuType[300], ManuMethod[300], FertOption=0, ManureAppHour[300];
...
for(int MFFC=1; MFFC<=TotalFarmFields; MFFC++)
{
	...
	for(i=1; i<=n; i++)
		{
			...
			fscanf(db, "%f %f %f %d %f %f", &ManuN[i], &ManureNH4[i], &ManureNO3[i], &ManureAppHour[i], &ManureDM[i],&ManurePH[i]);//added new manure parameters
		}
	fclose(db);
    ...
    for (i = 1; i<=n; i++) 
	{
        ...
        fprintf(fp,"%d %f %f %d %d %f %f %f %f %d %f %f\n", 
			day, 
			ManuAmount[i],
			ManuCN[i], 
			ManuType[i],
