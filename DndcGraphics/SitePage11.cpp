...
void SitePage11::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_GrazBiomass, m_GrazBiomass);
	DDX_Text(pDX, IDC_GrazCConsume, m_GrazCConsume);
	DDX_Text(pDX, IDC_GrazCExcrete, m_GrazCExcrete);
	//}}AFX_DATA_MAP
}
...
BOOL SitePage11::OnSetActive() 
{
	...
	for(i=0; i<=290; i++)
	{
		...
		AddFeed[i] = 0.0; FeedCN[i] = 0.0; Excreta[i] = 0; GrazBiomass[i] = 0; GrazCConsume[i] = 0; GrazCExcrete[i] = 0;
	}
	...
	i=1;
		...
		m_AddFeed = AddFeed[i];
		m_FeedCN = FeedCN[i];
		m_GrazBiomass = GrazBiomass[i];
		m_GrazCConsume = GrazCConsume[i];
		m_GrazCExcrete = GrazCExcrete[i];
...
void SitePage11::OnGAccept() 
{
	...
	i=m_GrazingID;		
		...
		AddFeed[i]=m_AddFeed;
		FeedCN[i]=m_FeedCN;
		GrazBiomass[i] = m_GrazBiomass;
		GrazCConsume[i] = m_GrazCConsume;
		GrazCExcrete[i] = m_GrazCExcrete;
	...
	for (i = 1; i<=m_GrazingN;i++) 
	{
		...
		fprintf(db, "%f %f %F\n", GrazBiomass[i], GrazCConsume[i], GrazCExcrete[i]);
	}
	fclose(db);
