...
BEGIN_MESSAGE_MAP(CGraphexView, CView)
	...
	//ON_MESSAGE(ID_RunSite, (LRESULT(AFX_MSG_CALL CWnd::*)(WPARAM,LPARAM))OnRunSite)
	//ON_COMMAND(ID_RunSite, (LRESULT(AFX_MSG_CALL CWnd::*)(WPARAM, LPARAM))OnRunSite)
	//ON_COMMAND(ID_RunSite, AFX_PMSG_CALL CWnd::OnRunSite)
	//ON_COMMAND(ID_RunSite, (AFX_PMSG) OnRunSite) //need2fix
...
void CGraphexView::OnDraw(CDC* pDC)
{
	...
	face.Format("%s\\library\\gleaner2.jpg", ROOTDIR);//face.Format("%s\\library\\gleaner2.jpg", ROOTDIR)
	sss=75.0;
#endif
#ifdef WIZARD
...
/*		dndc.Format("%s", "and"); 
...
#else 
	{
		...
		dndc.Format("%s", "DNDCv.CAN 9.6.1");

		hNewFont.CreateFont(45,0,0,0,FW_BOLD,0,0,0,
			                ANSI_CHARSET,OUT_DEFAULT_PRECIS,
							CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,
							DEFAULT_PITCH|FF_DONTCARE,"Courie New");
		...
		art.SetTextAlign(TA_CENTER); //art.SetTextAlign(TA_CENTER);

		art.SetTextColor(RGB(249,255,230));		
		art.TextOut(800, 88, dndc);//500,48,dndc

		art.SetTextColor(RGB(155,160,64));		
		art.TextOut(800, 90, dndc);//500,50,dndc
...
/*	char MB[200];
	FILE *db;
	sprintf(MB, "%s\\Inter\\OpenFile.txt", OUTPUT);
	db=fopen(MB, "w");
...
void CGraphexView::OnInputSite() //OK on main manu
{
	...
	DIR0.Format("%s\\Result", OUTPUT);
	_mkdir(DIR0);
	
	CString DIR1;
	DIR1.Format("%s\\Inputs", OUTPUT);
	_mkdir(DIR1);

	CString DIR2;
	DIR2.Format("%s\\Inter", OUTPUT);
	_mkdir(DIR2);
	
	DIR2.Format("%s\\Inter\\Management_1", OUTPUT);
	_mkdir(DIR2);
	
	DIR2.Format("%s\\Inter\\RegionFiles", OUTPUT);
	_mkdir(DIR2);

	CString DIR3;
	DIR3.Format("%s\\Record", OUTPUT);
	_mkdir(DIR3);
	...
	FILE *fpp;
			...
			sprintf(sitefile, "%s\\Inter\\DaymetFlag", OUTPUT);
...
*/
		//init error
		DDaymetFlag = 0;
		CreateInputFiles(DDaymetFlag, 0, "None", 1);//Normal start		
	}	
	...
	CString BF;
	BF.Format("%s\\Inter\\OnRunSiteFile.txt", OUTPUT);
	remove(BF);
}
...
void CGraphexView::OnMCresult() 
{
	...
	rcd.Format("%s\\Record\\Uncertainty_%s", OUTPUT, countyID);
	RCD = fopen(rcd, "r");
	if(RCD==NULL) 
	{
		CString mess;
...
void CGraphexView::OnBatch()
{
	...
	//defaults
	string user_output = ".\\Result";

	sprintf(ssROOTDIR, "%s", ".\\");
	
	sprintf(ssOUTPUT, "%s", user_output.c_str());
#ifdef CONSOLE
	OUTPUT = ssOUTPUT;
#endif
	...
	char BatchFileName[400];//batchfile name to read in...should make this an argument command if possible. 
	char output_dir[400];  //output folder
	char root_dir[400];
	char dailyMode[400];
	int DayRecord = 1;
	...
	while ((pos = cmd.find(delimeter)) != std::string::npos) {
		token = cmd.substr(0, pos);
		args.push_back(token);
		cmd.erase(0, pos + delimeter.length());
	}
	//last item was missed
	if (cmd.length() > 0) {
		args.push_back(cmd);
	}
	//change type
	...
	unordered_map<string, string> cmd_args = getOptionsFromArguments(argv.size(), argv.data());
	
	//custom root - mostly for debugging when you want a different root.
 	if (cmd_args.find("root") != cmd_args.end()) {
		//strcpy(root_dir, cmd_args.at("root").c_str());
		sprintf(ssROOTDIR, "%s", cmd_args.at("root").c_str());
		ROOTDIR = ssROOTDIR;
	}
	//custom output directory
	if (cmd_args.find("output") != cmd_args.end()) {
		strcpy(output_dir, cmd_args.at("output").c_str());
		sprintf(ssOUTPUT, "%s", output_dir);
		OUTPUT = ssOUTPUT;
	}

	//if Daily Record Batch Files a value of 1 is record a value of 0 is don't record daily
	if (cmd_args.find("daily") != cmd_args.end()) {
		//strcpy(output_dir, cmd_args.at("daily").c_str());
		//sprintf(ssOUTPUT, "%s", output_dir);
		sprintf(dailyMode, "%s", cmd_args.at("daily").c_str());//copies the cmd argument daily input by user into dailyMode
		DayRecord = stoi(dailyMode);//convert string to int using stoi
	}
	if (DayRecord > 1||DayRecord<0)DayRecord = 1;
	
	
	//batch file
	if (cmd_args.find("s") != cmd_args.end()) {
		strcpy(BatchFileName, cmd_args.at("s").c_str());
	}
	...
	sprintf(ssLIBRARY, "%s\\Library", ssROOTDIR); //need to fix.  make something debug worthy??
	LIBRARY = ssLIBRARY;

	sprintf(ssINPUTS, "%s\\Inputs", ssOUTPUT);
	INPUTS = ssINPUTS;

	sprintf(ssINTER, "%s\\Inter", ssOUTPUT);
#ifdef CONOLSE
	INTER = ssINTER;
#endif
	sprintf(ssINTERMANAGE, "%s\\Management", ssINTER);
#ifdef CONSOLE
	INTERMANAGE = ssINTERMANAGE;
#endif
	...
	DIR0.Format("%s\\Record\\Batch", OUTPUT);
	_mkdir(DIR0);

#ifdef CONSOLE

	
	int Monitor = 0;// , DayRecord = 1;
	...
	for (int i = 1; i <= FileNumber; i++)
	{
		...
		DIR0.Format("%s\\Record\\Batch\\Case%d-%s", OUTPUT, i, SiteName);
		_mkdir(DIR0);

		char BatchPass[300];
		sprintf(BatchPass, "%s", DIR0);

		DIR0.Format("%s\\Record\\Batch\\Case%d-%s\\CropSystem", OUTPUT, i, SiteName);
		_mkdir(DIR0);
		DIR0.Format("%s\\Record\\Batch\\Case%d-%s\\LivestockSystem", OUTPUT, i, SiteName);
		_mkdir(DIR0);

		CString BR;
		BR.Format("%s\\Inter\\BatchRecord", OUTPUT);
		...
		{
			CString note;
			note.Format("Cannot create file %s", BF);
			AfxMessageBox(note);
			exit(0);
		}
		fprintf(bf, "%s  %d  %d  %s", "1", Monitor, i, BatchPass);//fprintf(bf, "%s  %s  %s  %s", "1", Monitor,i,BatchPass);
		fclose(bf);
	

		OnRunSite();// Option 1: batch runs  //OnRunSite(1, Monitor, i, BatchPass)
...
#ifdef CONSOLE
#else
AfxMessageBox("Good. The Batch run is accomplished"); //msg box for succesful run ...turn off for console version.
#endif
	exit(0);
	
#else
	ScreenMonitor qq;
	int Monitor, DayRecord;
	...
	if(ww.DoModal()==IDOK)
	{
		...
		fscanf(region, "%s %d", BatchFileName, &DayRecord);		
		fclose (region);
		...
		for (int i=1; i<=FileNumber; i++)
		{
			...
			//Write out params for OnRunSite
...
void CGraphexView::OnSiteParameter() 
{
	...
	if(ww.DoModal()==IDOK)
	{
		CString SMC;
		FILE *smc;
		SMC.Format("%s\\Inter\\SiteMonteCarlo", OUTPUT);
		smc=fopen(SMC, "r");
		if(smc==NULL)
		{
			CString note;
		...
		for(j=1; j<=m_SampleSize; j++)
		{
			...
			CString note;
			FILE *mc;
			note.Format("%s\\Inter\\SiteMCinput", OUTPUT);
			mc=fopen(note, "w");
			if ( mc == NULL )
			{
				CString mess;
			...
			fprintf(bf, "%d  %s  %s  %s", 100+j, "0", "0", BatchPass);
			fclose(bf);
			//end params run for OnRunSite



			OnRunSite();// Option 2: site-Monte Carlo runs OnRunSite(100+j, 0, 0, BatchPass)
		}
	}
}
...
void CGraphexView::OnMapLayer() 
{
	...
	if(ww.DoModal()==IDOK)
	{
		...
		CString XZ;
		XZ.Format("%s\\Inter\\Layer1", OUTPUT);
		Layer1=fopen(XZ, "w");
		if (Layer1==NULL) 
			{
				CString note;
		...
		//FILE *Layer1;
		XZ.Format("%s\\Inter\\Layer2", OUTPUT);
		Layer1=fopen(XZ, "w");
		if (Layer1==NULL) 
			{
				CString note;
		...
		//FILE *Layer1;
		XZ.Format("%s\\Inter\\Layer3", OUTPUT);
		Layer1=fopen(XZ, "w");
		if (Layer1==NULL) 
			{
				CString note;
...
//	char BatchPass[300] = "//DNDC//RESULT//";

//Read in params for OnRunSite
	int option;
	int Monitor;
	int sample;
	char BatchPass[300] = { "" };
	...
	if (bf == NULL)
	{
		...
		//CString note;
		//note.Format("Cannot open file %s", BF);
		//AfxMessageBox(note);
		//exit(0);
	}
	else 
	{
		fscanf(bf, "%d  %d  %d  %s", &option, &Monitor, &sample, &BatchPass);
		fclose(bf);
	}
	//end params run for OnRunSite
...
if(option!=12345)
{
	...
	/* safer read: width-limit the string and check return values */
	/*
	if (fscanf(FileRun, "%59s", junkc) != 1) {
		fclose(FileRun);
		AfxMessageBox("Site file parse error: missing site name");
		return;
	}
	if (fscanf(FileRun, "%d", &TotalYears) != 1) {
		fclose(FileRun);
		AfxMessageBox("Site file parse error: missing TotalYears");
		return;
	}
	fclose(FileRun);
	*/
...
void CGraphexView::OnSiteModeTest() 
{
	...
	sprintf(SF, "%s\\Inputs\\Soil-A1", OUTPUT);
	sprintf(DF, "%s\\Inputs\\Soil-1", OUTPUT);
	...
	fprintf(bf, "%s  %s  %s  %s", "3", "0", "0", BatchPass);
	fclose(bf);
	//end params run for OnRunSite



	OnRunSite(); //OnRunSite(3, 0, 0, BatchPass)
}
...
void CGraphexView::OnYieldPredict() 
{
	...
	if(ww.DoModal()==IDOK)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////
			...
			CreateInputFiles(0, 10, "China", 1);//Drought run
			...
			fprintf(bf, "%s  %s  %s  %s", "10", "0", "0", BatchPass);
			fclose(bf);
			//end params run for OnRunSite

			OnRunSite();// Option 10: drought runs OnRunSite(10, 0, 0, BatchPass)
...
void CGraphexView::OnRunFarmComponent() 
{
	...
	fprintf(bf, "%s  %s  %s  %s", "12345", "1", "1", BatchPass);
	fclose(bf);
	//end params run for OnRunSite


	OnRunSite();//int option, int Monitor, int sample) OnRunSite(12345, 1, 1, BatchPass)
