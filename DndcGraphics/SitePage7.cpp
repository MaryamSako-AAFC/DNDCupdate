...
SitePage7::SitePage7() : CPropertyPage(SitePage7::IDD)
{
	...
	m_ManPoolRate=0.0;//manurePool settings
	m_ManPoolFrac=0.0;//manurePool settings
	m_UseManPool=FALSE;//use Manpool
	m_AutoManure = FALSE;
	//}}AFX_DATA_INIT
}
...
void SitePage7::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_ManPoolCHECK, m_UseManPool);
	DDX_Text(pDX, IDC_FracManPoolEdit, m_ManPoolFrac);
	DDX_Text(pDX, IDC_RateManPoolEDIT, m_ManPoolRate);
	DDX_Check(pDX, IDC_CHECK1, m_AutoManure);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(SitePage7, CPropertyPage)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_ManPoolCHECK, &SitePage7::OnBnClickedManpoolcheck)
	ON_BN_CLICKED(IDC_CHECK1, &SitePage7::OnBnClickedCheck1)
END_MESSAGE_MAP()
...
/*	if(m_ManN==0)
	...
	{*/
		for(i=1; i<=m_ManN; i++)
		{
			...
			fscanf(db, "%f %f %f %d %f %f %f %f %d %d", &ManuN[i], &ManureNH4[i], &ManureNO3[i], &ManureAppHour[i], &ManureDM[i], &ManurePH[i], &ManPoolFrac[i], &ManPoolRate[i], &UseManPool[i], &AutoManure);
		}		
	//}
	fclose(db);
	...
	i=1;
		...
		m_ManPoolFrac = ManPoolFrac[i];
		m_ManPoolRate = ManPoolRate[i];
		m_UseManPool = UseManPool[i];
		m_AutoManure = AutoManure;
		if (m_AutoManure == 0)IData.AutoManureCheck = 0;
		else { IData.AutoManureCheck = 1; }
	...
	//use autoManure
	if (IData.AutoManureCheck != 0 )
	{
		m_AutoManure = TRUE;
		IData.AutoManureCheck = 1;
	...
	{
		m_AutoManure = FALSE;
		IData.AutoManureCheck = 0;

	}
	m_AutoManure = IData.AutoManureCheck;
...
void SitePage7::OnManAccept() 
{
	...
	float properN,actualN;
	properN = m_ManAmount / m_ManC_N;
	actualN = m_kgN + mManureNH4;
	if (actualN> properN*1.05 || actualN < properN * 0.95)//(m_kgN != properN)
	{
		
			CString note;
			note.Format("Please Check: The Manure C content is %6.2f at a C:N of%6.2f should have an orgN + NH4 = %6.2f but the actual input N total is %6.2f",m_ManAmount,m_ManC_N,properN,actualN);
			AfxMessageBox(note);
			return;
		

	//	m_kgN = m_ManAmount / m_ManC_N;
	}
	/*
	float test1;
	test1 = m_kgN * m_ManC_N;
	if (m_ManAmount != test1)m_kgN = m_ManAmount / m_ManC_N;//hard coding organic N component
	*/
	...
	ManPoolFrac[i] = m_ManPoolFrac;
	ManPoolRate[i] = m_ManPoolRate;
	UseManPool[i] = m_UseManPool;
	AutoManure = m_AutoManure;
...
void SitePage7::OnKillfocusManAmount() 
{
	...
	float test;
	test= m_ManAmount / m_ManC_N;
	if(m_kgN != test )m_kgN = m_ManAmount / m_ManC_N;
	float test1;
	test1 = m_kgN * m_ManC_N;
	if(m_ManAmount!=test1)m_kgN = m_ManAmount / m_ManC_N;//hard coding organic N component
...
void SitePage7::demo() 
{
    ...
    if (m_ManN >= 0) 
	{
		for (i = 1; i<=m_ManN;i++) 
		{
			...
			txt.Format("%9.3f", ManPoolFrac[i]);
			m_ManList.SetItemText(i - 1, 9, txt);

			txt.Format("%9.3f", ManPoolRate[i]);
			m_ManList.SetItemText(i - 1,10, txt);

			txt.Format("%d", UseManPool[i]);
			m_ManList.SetItemText(i - 1, 11, txt);

			txt.Format("%d", AutoManure);
			m_ManList.SetItemText(i - 1, 12, txt);
...
void SitePage7::OnBnClickedManpoolcheck()
{
	// TODO: Add your control notification handler code here
}


void SitePage7::OnBnClickedCheck1()
{
	// TODO: Add your control notification handler code here
	//auto manure application check

	if (m_AutoManure == TRUE)
	{
