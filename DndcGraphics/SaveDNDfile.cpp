...
void NewSaveDatatoFile(char *fname, int SiteRegion)
{
	...
	int ManuMonth[300], ManuDay[300], ManuType[300], manu_method[300],ManureAppHour[300], UseManPool[300], AutoManure;
	...
	if(IData.Sks<0.015) IData.Sks = 0.015;//default 0.015 was 0.008
...
#ifdef MANURE
	//specify the outputs to write to the savedndfile
	...
	float m_BodyWeight, m_DryMatterFraction, m_MilkProtein, m_DaysInMilk;//new
	float m_NDF, m_ADF, m_Lignin, m_FAT, m_ASH;//new
	int m_EmpiricalModel, m_FeedAdditiveType;//new
	float m_FeedAdditiveRate;//new
	...
	float LagoonToField[30]={0.0,0.0}, LagoonToMarket[30]={0.0,0.0}, LagoonToRemain[30]={0.0,0.0}, LagoonManureAplDepth[30]={0,0};
	int LagoonCover, LagoonRain, LagoonDrainTimes, LagoonDrainMonth[30]={0,0}, LagoonDrainDay[30]={0,0}, LagoonManureAplMethod[30]={0,0}; 
	int m_LagoonYear;//new
	int m_LagoonNumber, m_LagoonID, m_SLagoonID;//new sometime for multi lagoon
	...
	float m_UL2, m_UL3, m_UL4, m_FL2, m_FL3, m_FL4;//new for 2nd lagoon fraction
	...
	//write to number of feedlots including new parameters.
	...
	for(int m_FeedlotYear=1; m_FeedlotYear<=IData.years; m_FeedlotYear++)
	{
		...
		for(i=1; i<=m_FeedLots; i++)//for number of feedlots
		{
			CString CM;
			FILE* cm;
			...
			//new
			fscanf(cm, "%d", &m_EmpiricalModel); //UCD empirical model calculating GHG and Manure
			fprintf(fpo, "%-40s %30d\n", "________EmpiricalModel", m_EmpiricalModel);

			fscanf(cm, "%d", &m_FeedAdditiveType); //FeedAdditive for CH4 mitigation
			fscanf(cm, "%f", &m_FeedAdditiveRate); //FeedAdditive for CH4 mitigation
			...
			else
			{
				...
				//new
				fscanf(cm, "%f  %f  %f  %f", &m_BodyWeight, &m_DryMatterFraction, &m_MilkProtein, &m_DaysInMilk);
				fscanf(cm, "%f  %f  %f  %f %f", &m_NDF, &m_ADF, &m_Lignin, &m_FAT, &m_ASH);
				...
				//new
				fprintf(fpo, "%-40s %30.4f\n", "________Body_weight", m_BodyWeight);
				fprintf(fpo, "%-40s %30.4f\n", "________Dry_matter_fraction", m_DryMatterFraction * 0.01);
				fprintf(fpo, "%-40s %30.4f\n", "________Milk_protein_percent", m_MilkProtein);
				fprintf(fpo, "%-40s %30.4f\n", "________Days_in_milk", m_DaysInMilk);
				fprintf(fpo, "%-40s %30.4f\n", "________NDF_percent", m_NDF);
				fprintf(fpo, "%-40s %30.4f\n", "________ADF_percent", m_ADF);
				fprintf(fpo, "%-40s %30.4f\n", "________Lignin_percent", m_Lignin);
				fprintf(fpo, "%-40s %30.4f\n", "________Fat_percent", m_FAT);
				fprintf(fpo, "%-40s %30.4f\n", "________Ash_percent", m_ASH);
				fprintf(fpo, "%-40s %30d\n", "________Feed_additive_type", m_FeedAdditiveType);
				fprintf(fpo, "%-40s %30.4f\n", "________Feed_additive_rate", m_FeedAdditiveRate);
	...
	for (int m_LagoonYear = 1; m_LagoonYear <= IData.years; m_LagoonYear++)
	{
		CString CM;
		FILE* cm;

		CM.Format("%s\\Inter\\ManureLagoon_%d.txt", OUTPUT, m_LagoonYear);
		cm = fopen(CM, "r");
		if (cm == NULL) note(0, CM);


		//read ManureLagoon.txt (interim file) to write out to Saved DND file. will need to write out m_LagoonYear

		//for(i=1; i<=m_LagoonYear; i++)//for number of lagoons
	...
	CString CM;//added for Lagoon LOOP
...
if(IData.ProfileType==0)
{
	...
	fprintf(fpo,"%-40s %30d\n", "__Homo_Slow_Water_Flow", IData.CheckHomo1);//Homogeneous Water Flow
	fprintf(fpo,"%-40s %30d\n", "__None", 0);
	fprintf(fpo,"%-40s %30d\n", "__None", 0);
	fprintf(fpo,"%-40s %30d\n", "__None", 0);
	fprintf(fpo,"%-40s %30d\n", "__None", 0);
	...
	{		
		...
		for(yyy=1; yyy<=IData.db_CycleSpan[rrr]; yyy++)
		{
			...
			for(int ccc=1; ccc<=IData.db_YrCrops[rrr][yyy]; ccc++)
			{
					...
					fscanf(db, "%f %f", &IData.db_LAImax[rrr][yyy][ccc], &IData.db_FrostKill[rrr][yyy][ccc]);
					fscanf(db, "%f %f %f %f", &IData.db_AlfCHRMX[rrr][yyy][ccc], &IData.db_ALFCDRMX[rrr][yyy][ccc], &IData.db_ALFCTMX[rrr][yyy][ccc], &IData.db_ALFPDFMX[rrr][yyy][ccc]);
					fscanf(db, "%f %f", &IData.db_Root1[rrr][yyy][ccc], &IData.db_Root2[rrr][yyy][ccc]);
					fscanf(db, "%f %f", &IData.db_AGResp1[rrr][yyy][ccc], &IData.db_AGResp2[rrr][yyy][ccc]);
					fscanf(db, "%f %f %f %f", &IData.db_cropGRate[rrr][yyy][ccc], &IData.db_cropTime[rrr][yyy][ccc], &IData.db_RUEHI[rrr][yyy][ccc], &IData.db_DelayP[rrr][yyy][ccc]);
    ...
    for (rrr = 1; rrr<=IData.db_Rotation_Number;rrr++) 
	{
        ...
        for (j = 1; j<=IData.db_CycleSpan[rrr];j++) 
		{
            ...
            for (k = 1; k<= IData.db_YrCrops[rrr][j];k++)
			{
				...
				fprintf(fpo,"%-40s %30.4f\n", "______Root_Depth", IData.db_Vascularity[rrr][j][k]);//rooting depth replacing vascularity
				...
				fprintf(fpo, "%-40s %30.4f\n", "______LAI_Maximum", IData.db_LAImax[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______FrostKill_Temperature", IData.db_FrostKill[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______ALF_CHRMX", IData.db_AlfCHRMX[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______ALF_CDRMX", IData.db_ALFCDRMX[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______ALF_CTMX", IData.db_ALFCTMX[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______ALF_PDFMX", IData.db_ALFPDFMX[rrr][j][k]);
				//need to add some new parameters for root respiration.
				fprintf(fpo, "%-40s %30.4f\n", "______RootResp1", IData.db_Root1[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______RootResp2", IData.db_Root2[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______AGResp1", IData.db_AGResp1[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______AGResp2", IData.db_AGResp2[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______cropGrowthRate", IData.db_cropGRate[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______cropGrowthTiming", IData.db_cropTime[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______RUEonHarvestIndex", IData.db_RUEHI[rrr][j][k]);
				fprintf(fpo, "%-40s %30.4f\n", "______DelayedPhenology", IData.db_DelayP[rrr][j][k]);
				//fprintf(fpo,"%-40s %30d\n", "______None", 0);//was 5 total
				//fprintf(fpo,"%-40s %30d\n", "______None", 0);
				//fprintf(fpo,"%-40s %30d\n\n", "______None", 0);
            }
...
fprintf(fpo,"----------------------------------------\n");
			...
			for(i=1; i<=n; i++)
				{
					...
					fscanf(db, "%f %f %f %d %f %f %f %f %d %d", &ManuN[i], &ManureNH4[i], &ManureNO3[i], &ManureAppHour[i], &ManureDM[i], &ManurePH[i], &ManPoolFrac[i], &ManPoolRate[i], &UseManPool[i], & AutoManure);// for manure file was never turned off.
				}
			fclose(db);
            ...
            for(k=1; k<= n; k++) 
			{
				...
				fprintf(fpo, "%-40s %30d\n", "______UseManPool", UseManPool[k]);
				fprintf(fpo, "%-40s %30.4f\n", "______ManPoolResistantFrac", ManPoolFrac[k]);
				fprintf(fpo, "%-40s %30.4f\n", "______ManPoolDecompRate", ManPoolRate[k]);
				fprintf(fpo, "%-40s %30d\n", "______AutoManure", AutoManure);
		//		fprintf(fpo,"%-40s %30d\n", "______None", 0);
            }
...
fprintf(fpo,"----------------------------------------\n");
			...
			fclose(db);
			if (m_GroundOutflowIntensity <= 0.0)
			{
				m_GroundOutflowIntensity = 0.0000;
			}
...
fprintf(fpo,"----------------------------------------\n");	
			...
			db=fopen(DB, "r");
			if(db==NULL) note(0,DB);
			fscanf(db, "%d", &n);//scan the number of controlled drainage
			for (i = 1; i<=n;i++) 
			{
...
fprintf(fpo,"----------------------------------------\n");
            ...
            for(k=1; k<=n; k++)
			{
				...
				fprintf(fpo, "%-40s %30.4f\n", "______Graz_Biomass_Impact", GrazBiomass[k]);
				fprintf(fpo, "%-40s %30.4f\n", "______GrazCConsume", GrazCConsume[k]);
				fprintf(fpo, "%-40s %30.4f\n", "______GrazCExcrete", GrazCExcrete[k]);
...
fprintf(fpo,"----------------------------------------\n");
	...
	fprintf(fpo, "%-40s %30.4f\n", "______N2N2OFactor", IData.N2N2OF);
	fprintf(fpo, "%-40s %30d\n", "______InitializeAtPlanting", IData.SpinUpCheck2);
	fprintf(fpo, "%-40s %30d\n", "______UseOBSYield", IData.OBSYield);
	if (IData.OBSYield == 1)
	{
		fprintf(fpo, "%-40s %40s\n", "______OBSYieldFileName", IData.OBSYieldFileName);
	}
	else
	{
		fprintf(fpo, "%-40s %30s\n", "______OBSYieldFileName", "");
		IData.OBSYieldFileName = " ";
		
	}
	fprintf(fpo, "%-40s %30d\n", "______AutoPlant", IData.AutoPlant);
	fprintf(fpo, "%-40s %30d\n", "______PrefFlowCheck", IData.PrefFlowCheck);
	fprintf(fpo, "%-40s %30.4f\n", "______PrefFlowDepth", IData.PrefFlowDepth);
	fprintf(fpo, "%-40s %30.4f\n", "______PrefFlowRecovery", IData.PrefFlowRecov);
	fprintf(fpo, "%-40s %30d\n", "______UseDynamicLayer", IData.DynLayerCheck);
	fprintf(fpo, "%-40s %30.4f\n", "______UserLayerDepth", IData.DynLayerEdit);
