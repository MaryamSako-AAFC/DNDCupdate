...
void ReadDNDfile(char *fname, int UseID)
{
	...
	float m_LagoonCapacity, m_LagoonSurfaceArea, LagoonToField[10], LagoonToDigester[10], LagoonRemain[10], LagoonManureAplDepth[10];
...
if(db_Type==53)//change parameters of broccoli
{
	...
	db_Water = 150.0;
				...
				for(x=1; x<=3; x++) fscanf(fp,"%s %d", notes, &jd);//normally x<=5 until i added additional 2 lines
			...
			fscanf(fp,"%s %f", notes, &m_GroundOutflowIntensity);				
