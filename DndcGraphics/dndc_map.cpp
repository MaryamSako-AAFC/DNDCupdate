...
/*	for(i=0;i<=12;i++)
	...
	char SFF[200];
	FILE *sff;
	sprintf(SFF, "%s\\Inter\\FileReview", OUTPUT);
...
//		ASSERT_VALID(pDoc);
	...
	for(i=1;i<=20;i++)	
	{
		float NewFlux=Interval*(float)powf(5.0,(float)i);
	...
	if(TU==1)
	{
		...
		fprintf(ssf, "\n");
		fprintf(ssf, ",,,");
		for(int a=6;a<=45;a++) fprintf(ssf, "%-15s, %-15s, %-15s, ", "min", "max", "average");
		fprintf(ssf, "\n");
...
void DNDC_Map::OnSelchangeSelectCrop() 
{
	...
	CString SSFF;
	SSFF.Format("%s\\Record\\Region\\crop_summary_%d.csv", OUTPUT, SelectedCrop);
	FILE *ssf;
	ssf = fopen(SSFF, "w");
	...
	CString CCRR;
	FILE* ccrr;
	CCRR.Format("%s\\Record\\Region\\Continent_crop_yield.csv", OUTPUT);
...
void DNDC_Map::OnWeighIrri() 
{
	...
	FILE *sff;
	sprintf(SFF, "%s\\Inter\\IrriFileReview", OUTPUT);
